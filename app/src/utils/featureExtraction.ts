import type { HandFrame, HandPoseDetection, HandKeypoint } from "../types";
...
const normalizeHandedness = (hand: HandPoseDetection | undefined): string | undefined => {
  if (!hand?.handedness) return undefined;
  return hand.handedness.toString().trim().toLowerCase();
};

const ensureKeypoints = (points: HandKeypoint[] | undefined): HandKeypoint[] => {
...
const coerceLegacyFrame = (frame: HandFrame): HandPoseDetection[] => {
  if (frame.hands && frame.hands.length) {
    return frame.hands.map((hand): HandPoseDetection => ({
      confidence: sanitizeNumber(hand.confidence),
      handedness: hand.handedness,
      keypoints: ensureKeypoints(hand.keypoints),
      keypoints3D: ensureKeypoints(hand.keypoints3D),
    }));
  }
  ...
  return [
    {
      confidence: sanitizeNumber(legacy.confidence),
      handedness: undefined,
      keypoints: ensureKeypoints(legacy.keypoints),
      keypoints3D: ensureKeypoints(legacy.keypoints3D),
    },
  ];
};
...
const orderHands = (hands: HandPoseDetection[] | undefined): Array<HandPoseDetection | undefined> => {
  ...
  const leftIndex = hands.findIndex((hand) => normalizeHandedness(hand) === "left");
  if (leftIndex >= 0) {
    slots[0] = hands[leftIndex];
  }

  const rightIndex = hands.findIndex((hand, idx) => idx !== leftIndex && normalizeHandedness(hand) === "right");
  if (rightIndex >= 0) {
